...
ckpt_path_dict = dict()
ckpt_root_dir = './log/per_gap_8_layer_12_heads_no_dropout_unnormalize_max_level_4_mse_0.975_miss_irr_att_d_128_model_d_1024_large_train/ckpt'
...
loss = run_imputation(model, test_data.repeat(10,1,1), args.num_missing, ckpt_dict, confidence=params.confidence , max_level=params.max_level, batch_size=10, fig_path = os.path.join(params.exp_dir, 'impute'), 
                    save_all_imgs=args.save_fig, dataset=params.dataset, train_mean=train_mean, test_mean=test_mean)

output_str = 'Testing_Loss: %4f' % (loss)
print(output_str)
os.mkdir(os.path.join(params.exp_dir, 'impute_%4f' %(loss)))